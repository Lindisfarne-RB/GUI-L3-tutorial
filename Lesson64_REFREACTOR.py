...
In our GUI window, we have the area that displays the account balances, and at the moment that is hard coded so we want to fix that up.

We're going to rewrite the update_balance() function so that it gets all of the data from the objects themselves and then updates the GUI label by storing the data in the account_details variable we already have.

We're also going to change the format a little so we can include the goals and a total balance.

CREATE
First of all, let's delete the line where the account_details are set in the GUI code, around line 82.
Click CHECK to make sure you've deleted the right line.
TIPS
Nothing fancy will happen yet if you click RUN, we are just preparing our code.
