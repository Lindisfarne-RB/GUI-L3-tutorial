'''Updating the GUI
The last step in getting our app working is to update the GUI by calling update_balance() to update the account balances, and by clearing the amount entry so that it's easy to carry out another action.

We'll also add an exception to our try block so that if the user types text into the amount entry we can give them an error message in the action_feedback label.

CREATE
Under the relevant comment in the manage_action() function, call update_balance(). This should be indented to the same level as the for loop so it happens after either depositing or withdrawing.
Below this, set() the amount variable to "" to clear the entry field.
Under the exception comment, aligned with try add except ValueError: for any text input.
Inside the exception, set action_feedback to Please enter a valid number.
Click RUN and try depositing and withdrawing some money from the accounts.'''
####################  IMPORTS  #######################
from tkinter import *
