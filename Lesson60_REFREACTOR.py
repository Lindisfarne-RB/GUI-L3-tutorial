'''Cleaning up the functions and setup code
We have all of our code in one place now, but we need to tidy it up a bit. In the last 10 lessons, we ended up with a working program, but we had figured out that the update_balance() function was very repetitive, and would get even more repetitive if we wanted to add another account.

Now that we have built an account class, we will rewrite parts of our program to work in an object-oriented way.

We will no longer need the balance variables like savings_balance as this information is stored in each object's attributes.

We will also rewrite the update_balance() function and add some others to tie our class code to the GUI.
CREATE
Let's clean up our code a bit!

Find the old account balance variables in the FUNCTIONS AND SETUP section and delete all 3 of them and the comment.
Delete everything inside the update_balance() function and replace it with the word pass  - we want to keep the function, but we won't rewrite it just yet.
