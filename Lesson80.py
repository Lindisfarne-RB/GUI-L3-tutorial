'''Polishing up the UI and testing the functions
Cool! Our app is working now. We can deposit or withdraw money from any of the accounts and the balances will update.

We still have the word Testing in the action_feedback label at the start though, and we should really test that it all works as expected, including the error messages.

CREATE
Delete the line where the action feedback is set to Testing.
Now, click RUN and carry out the following tests:

Deposit a positive number into the Savings account.
Deposit a positive number into another account.
Deposit a negative number into Savings.
Deposit a word into Savings.
Make sure Savings has a positive balance then withdraw a positive number that is less than the balance.
Withdraw a positive number that is greater than the balance.
