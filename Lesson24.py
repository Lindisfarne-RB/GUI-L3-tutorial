'''Adding an account Combobox to our goal tracking GUI
We've set up a label for our Combobox, so now let's get the variable and list we need and create the Combobox itself.

CREATE
Complete these tasks under the relevant comments:

Create a list called account_names with the following 3 account/savings target names: Savings, Phone, Holiday.
Create a string variable called chosen_account.
Set this string variable to the first option in the account_names list.
Create a ttk Combobox called account_box using the chosen_account variable and set it to "readonly".
Set the ['values'] for the Combobox to the list of account_names.
Place the account_box in row 3, column 1 next to its label, with 10px of padding on the sides and 3px top and bottom.'''
