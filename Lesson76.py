'''18.1
Planning a function to manage the action
Alright, we are now in the final stages of making our app. All that is left to do is write one more function that will manage the user's action. It will run when the submit button is clicked, and determine whether to call the deposit_money() or withdraw_money() function.

We will also add some validation to make sure the user can't enter text into the amount field.

Here's an outline:
Try
  For each account in the list
    If it is the chosen account
      If the chosen action is "deposit"
        Run the deposit function on the account
      Else
        Run the withdraw function on the account

  Update the balances
  Clear the amount input

Except
  Ask user to enter a valid number
CREATE
Under the relevant comment, create a new function called manage_action() with no parameters.
Inside the function, add a try: block.
Scroll down to the submit button near the end and change the command argument so that it calls manage_action instead of update_balance.
(Optional) Add a pass  inside the try block if you want to run the code now without indentation errors.
TIPS
We'll need to add a bit more code before this works when you click RUN so head to the next task when you're done!
'''
...
happy_image = PhotoImage(file="smiley.png")
