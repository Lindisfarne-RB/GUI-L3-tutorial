'''Combining the details from each account
Now that we have some variables set up, let's think about the logic of this function and how we want this next part to work. Here is some pseudocode outlining the basic idea:
...
...We want to add each account's details to the balance_string which will go in the label, and add the balance to the total_balance. We will need to use the object's get_progress() method here as well.

CREATE
Under the relevant comment, write a for loop statement that will loop through each account in account_list.
Inside the loop, call the account's progress() method and store the returned value as progress.
Add to balance_string this string: {}: ${} - {}% of ${} goal and .format() it with the correct values for each placeholder.
Add the account's balance to the total_balance.
(Optional) Format the account balance output to have 2dp and the progress to have 0dp.'''
